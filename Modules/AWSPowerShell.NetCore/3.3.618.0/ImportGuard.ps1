function AWSToolsImportGuard() {
    ...
    $printGuidance = $true
    $guidance = " AWS.Tools.Common (and other AWS.Tools.* modules like AWS.Tools.EC2 or AWS.Tools.S3) is the new modularized version of AWS Tools for PowerShell, currently available in prerelase. AWSPowerShell.NetCore is the suggested version for all systems running PowerShell 3.0+ with .NET Framework 4.7.2+ or PowerShell 6.0+. AWSPowerShell is the legacy module for older systems which cannot be updated to PowerShell 3.0+ with .NET Framework 4.7.2+ or to PowerShell 6.0+."

    foreach ($rootModule in $rootModules) {
        $version = Get-Module -Name $rootModule | Select-Object Version
        if ($version) {
            $message = "$rootModule version $($version.Version) is currently imported. Only a single version of a single variant (AWSPowerShell, AWSPowerShell.NetCore and AWS.Tools.Common) of AWS Tools for PowerShell can be imported at any time."
    ...
    foreach ($rootModule in $rootModules) {
        if (Get-Module -Name $rootModule -ListAvailable) {
            $count = $count + 1
        }
    }

    if ($count -gt 1) {
        $message = "Multiple variants (AWSPowerShell, AWSPowerShell.NetCore and AWS.Tools.Common) of AWS Tools for PowerShell are currently installed. Please run 'Get-Module -Name AWSPowerShell,AWSPowerShell.NetCore,AWS.Tools.Common -ListAvailable' for details. To avoid problems with cmdlet auto-importing, it is suggested to only install one variant."
        if ($printGuidance) {
            $printGuidance = $false
            $message = $message + $guidance
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDmf8BrhN8l5n9W
# CBZqpWeKdKoz87s/4V7SL7z702FFTKCCF0cwggS5MIIDoaADAgECAhArd4OFAE9M
...
# DAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgf3z0CNKIWeeTMFcVc8+lofLF
# cfMOGirUPhvVjLB3JUwwDQYJKoZIhvcNAQEBBQAEggEAUX0ti4y4qSOyd7lXp7eA
# lylKhFEfqopT1zlsG9MERfLNn7r1TPGqgFm9dpNGPKeMDqsej4DsQYP98W+QJZXP
# I6fTzKGvmT4LbBjg+Xc7ovQvYwGp16OmdqlsNcvabsAH0MJRyh3HOpKbNnLiZj/q
# Ec6URQEiLi83nLWQ4j4bRKfWv+sv+HlyGQ62W7Vt7IZ0IIzG6Ha3IgWqnC7UAZnu
# Iid99IUmLLrtOKa4H5IIT8hOfd3nrN3eZ270K45y9+iBHPlgNP9LSRmil23foetU
# NKHw3ZuMQYQP1X+Uwcs155OLg89GAeCZZjqKIktb2sDYRL+xt8etrnQNEkxiOQIL
# OKGCAg8wggILBgkqhkiG9w0BCQYxggH8MIIB+AIBATB2MGIxCzAJBgNVBAYTAlVT
# MRUwEwYDVQQKEwxEaWdpQ2VydCBJbmMxGTAXBgNVBAsTEHd3dy5kaWdpY2VydC5j
# b20xITAfBgNVBAMTGERpZ2lDZXJ0IEFzc3VyZWQgSUQgQ0EtMQIQAwGaAjr/WLFr
# 1tXq5hfwZjAJBgUrDgMCGgUAoF0wGAYJKoZIhvcNAQkDMQsGCSqGSIb3DQEHATAc
# BgkqhkiG9w0BCQUxDxcNMTkxMTA0MTY0ODQwWjAjBgkqhkiG9w0BCQQxFgQUI3yp
# SMxSnTSkm18UTrAtQ5i9gfIwDQYJKoZIhvcNAQEBBQAEggEAaIdZqGHfFgyDOPhk
# gbWRuiVPP2LXG+qO1U9bny59nCQfQFnJasu8XtdmIBze0K+v7ewcgniPd2WcC4xN
# OJOveav0PutEhK3E8oNojg4JOc3Y4Mn7oFQDXgwOxovCI/pOI23+A0mhQY6l/Pn/
# 7S5NlHhk4UR+B+o353cv8vjCunxX/bT2uTWXqXfThPTlDv6yqptRDCGxHKkLlxcW
# ZyadTvjo38oRRRDN+yhhQKCeuKNBZKHq7SXVNUB5XZnoRtJyNx5ZwHFyOgVPvQid
# QMV7/gciOoDgPxynJ7QqoqEd5IIAxdGOBk2pe4ahI0wkTZiJygc1LLZbDBKbnrI4
# DTikcA==
